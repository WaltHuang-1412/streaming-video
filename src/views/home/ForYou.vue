...
swiper.for-you(v-if="isArray(data)" direction="vertical" @slide-change="handleSlideChange")
  template(v-for="(item,index) in data")
    swiper-slide
      ...
      .swiper-slide__video(:id="`for-you${index}`")
      .swiper-slide__shadow(@click="handleShadowClick")
Teleport(v-if="selectedVideo.id >= 0" :to="`#for-you${selectedVideo.id}`")
...
export default defineComponent({
  ...
  setup() {
    ...
    const initialize = async () => {
      await getData()
      chooseVideo(0)
      if (route.name === routeList.homeForYou) {
    ...
    watch(
      () => route.name,
      async (newValue) => {
        if (newValue === routeList.homeForYou) {
          selectedVideo.autoPlay = true
          handlePlayClick()
        } else {
          handlePauseClick()
